...
def stemming_on_sentence(line, aggressive=False):
    return ' '.join([cz_stem(word, aggressive=aggressive) for word in line.split()])


def stemming_on_word(word, aggressive=False):
    return cz_stem(word, aggressive=aggressive)


def test():
    # if len(sys.argv) != 2 or sys.argv[1] not in ("light", "aggressive"):
    #    sys.exit("usage: {} light|aggressive".format(sys.argv[0]))
    # aggressive = sys.argv[1] == "aggressive"  # for line in sys.stdin:
    aggressive = False

    with open("example.txt", encoding="utf-8") as f:
        with open("output.txt", "w", encoding="utf-8") as ff:
            for line in f.readlines():
                ff.write(' '.join([cz_stem(word, aggressive=aggressive) for word in line.split()]) + "\n")
